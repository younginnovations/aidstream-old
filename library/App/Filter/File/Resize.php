 ...
 */
class App_Filter_File_Resize implements Zend_Filter_Interface
{
    ...
    protected $_adapter = 'App_Filter_File_Resize_Adapter_Gd';
     ...
     * @return App_Filter_File_Resize An instance of this filter
     */
    public function __construct($options = array())
    {
        ...
        if (isset($options['adapter'])) {
            if ($options['adapter'] instanceof App_Filter_File_Resize_Adapter_Abstract) {
                $this->_adapter = $options['adapter'];
            } else {
                $name = $options['adapter'];
                if (substr($name, 0, 33) != 'App_Filter_File_Resize_Adapter_') {
                    $name = 'App_Filter_File_Resize_Adapter_' . ucfirst(strtolower($name));
                }
                $this->_adapter = $name;
            }
        }
    ...
    {
        if ($this->_adapter instanceof App_Filter_File_Resize_Adapter_Abstract) {
            return;
        } else {
