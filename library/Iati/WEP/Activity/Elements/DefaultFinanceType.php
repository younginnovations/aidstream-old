    ...
    protected $attributes = array('id', 'code', 'text', 'xml_lang');
     ...
     protected $attributes_html = array(
        ...
        'text' => array(
            ...
            'name' => 'text',
            'label' => 'Text',
            'html' => '<textarea rows="2" cols="20" name="%(name)s" %(attrs)s>%(value)s</textarea><div class="help default_finance_type-text"></div>',
                    'attrs' => array('class' => array('form-text')),
        ),
        'code' => array(
            'name' => 'code',
            'label' => 'Finance Type Code',
            'html' => '<select name="%(name)s" %(attrs)s>%(options)s</select><div class="help default_finance_type-code"></div>',
            'options' => '',
        ...
        'xml_lang' => array(
            'name' => 'xml_lang',
            'label' => 'Language',
            'html' => '<select name="%(name)s" %(attrs)s>%(options)s</select><div class="help default_finance_type-xml_lang"></div>',
            'options' => '',
            'attrs' => array('class' => array('form-select'))
        )                   
     );
...
//        $this->checkPrivilege();
    ...
    public function checkPrivilege()
    {
        ...
        if (!Zend_Registry::get('acl')->isAllowed($userRole, $resource, 'DefaultFinanceType')) {
